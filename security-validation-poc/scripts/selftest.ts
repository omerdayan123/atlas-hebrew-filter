...
const CASES: Case[] = [
  ...
  { text: "תור לרופא", expect: "ALLOW", note: "doctor appointment" },
  {
    text: "לשכת אלוף",
    expect: "BLOCK",
    note: "office + senior rank (combination)",
  },
  {
    text: "בלשכת האלוף",
    expect: "BLOCK",
    note: "same, with prefixes (normalization)",
  },
  { text: "יחידת 8200", expect: "BLOCK", note: "sensitive unit number" },
  {
    text: "חמל צפון",
    expect: "BLOCK",
    note: "ops room + region (combination)",
  },
];
...
async function main() {
  ...
  const payload = (await (
    await fetch(`${BASE}/api/security/sync`)
  ).json()) as SyncPayload;
  ...
  for (const q of ["מר", "חד", "תו"]) {
    const r = await (
      await fetch(
        `${BASE}/api/security/suggest?q=${encodeURIComponent(q)}&limit=5`,
      )
    ).json();
    console.log(
      `q="${q}"  total=${r.total}  tookMs=${r.tookMs}  -> ${r.suggestions.join(", ")}`,
    );
  }
  ...
  const inputs: { label: string; text: string }[] = [
    ...
    {
      label: "50 words",
      text: Array(5).fill(word).join(" ").split(" ").slice(0, 50).join(" "),
    },
    {
      label: "100 words",
      text: Array(10).fill(word).join(" ").split(" ").slice(0, 100).join(" "),
    },
  ];
  for (const inp of inputs) {
    ...
    console.log(
      `${pad(inp.label, 9)} ${avg.toFixed(4)} ms/validate  (${(1000 / avg).toFixed(0)}/sec)`,
    );
  }
  ...
  console.log(
    `false positives: ${fp}/${TRIALS} (${((fp / TRIALS) * 100).toFixed(3)}%) — target ~0.1%`,
  );
