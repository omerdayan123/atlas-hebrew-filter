...
function refreshIfChanged(): void {
  ...
  if (current !== signature) {
    ...
    console.log(
      `[sync] data changed — rebuilt index, version=${data.payload.version}`,
    );
  }
}
...
app.get("/api/security/suggest", (req, res) => {
  ...
  const result =
    norm.length >= 2
      ? suggest(data.suggestionIndex, norm, limit)
      : { suggestions: [], total: 0 };
...
app.get("/api/admin/lists/:type", (req, res) => {
  ...
  if (
    !["blacklist", "whitelist", "combinations", "safe_combinations"].includes(
      type,
    )
  ) {
    res
      ...
      .json({
        error:
          "type must be blacklist, whitelist, combinations, or safe_combinations",
      });
    return;
  }
  ...
  if (type === "blacklist") {
    ...
    items = data.payload.combinations.map((c) => ({
      term: c.phrase,
      normalized: c.normalizedPhrase,
      category: c.riskLabel,
    ...
    items = data.payload.safeCombinations.map((sc) => ({
      term: sc.phrase,
      normalized: sc.normalizedPhrase,
      notes: sc.notes,
    }));
  } else {
    // Whitelist — use the suggestion index (already sorted).
    items = data.suggestionIndex.map((s) => ({
      term: s.term,
  ...
  if (q)
    items = items.filter((i) => i.term.includes(q) || i.normalized.includes(q));
...
app.delete("/api/admin/lists/:type", (req, res) => {
  ...
  res.json({
    ok: true,
    message: `"${term}" removed from ${type}`,
    version: data.payload.version,
  });
});
